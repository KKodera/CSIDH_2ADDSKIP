...
long long bench_isogeny[2][primes_num];
...
int main()
{
  ...
  for (long long old = 0;old < 2;++old) {
    ...
    if (old)
      for (long long lpos = 0;lpos < primes_num;++lpos) {
        ...
        bench_isogeny[old][lpos] = fp_mul_count - bench;
      }
    else
      for (long long lpos = 0;lpos < primes_num;++lpos) {
        bench = fp_mul_count;
    ...
    for (long long key = 0;key < KEYS;++key)
      for (long long t = 0;t < TIMINGS;++t) {
        ...
        bench_action[old][key][t] = fp_mul_count - bench;

        bench = fp_mul_count;
        ok = validate(&pub[key][t]);
        bench_validate[old][key][t] = fp_mul_count - bench;
        assert(ok);
      }
    for (long long key = 0;key < KEYS;++key)
      for (long long t = 0;t < TIMINGS;++t)
  ...
  for (long long lpos = 0;lpos < primes_num;++lpos)
    printf("l %lld mults %lld baseline %lld\n",primes[lpos],bench_isogeny[0][lpos],bench_isogeny[1][lpos]);
