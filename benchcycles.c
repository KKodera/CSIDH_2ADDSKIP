...
long long bench_isogeny[3][primes_num][TIMINGS];
...
int main()
{
  ...
  for (long long type = 0;type < 3;++type) {
    ...
    if (type == 0)
      for (long long lpos = 0;lpos < primes_num;++lpos)
        for (long long t = 0;t < TIMINGS;++t) {
          ...
          bench_isogeny[type][lpos][t] = getticks() - bench;
        }
    else if (type == 1)
      for (long long lpos = 0;lpos < primes_num;++lpos)
        for (long long t = 0;t < TIMINGS;++t) {
      ...
      for (long long lpos = 0;lpos < primes_num;++lpos)
        for (long long t = 0;t < TIMINGS;++t) {
          bench = getticks();
          xISOG_2ADDSkip(&A[lpos][t],&P[lpos][t],&K[lpos][t],primes[lpos],nfor2addskip[lpos]);
          bench_isogeny[type][lpos][t] = getticks() - bench;
        }
    for (long long lpos = 0;lpos < primes_num;++lpos)
      for (long long t = 0;t < TIMINGS;++t)
    ...
    for (long long key = 0;key < KEYS;++key) {
      for (long long t = 0;t < TIMINGS;++t) {
        ...
        bench_action[type][key][t] = getticks() - bench;

        bench = getticks();
        ok = validate(&pub[key][t]);
        bench_validate[type][key][t] = getticks() - bench;
        assert(ok);
      }
    }
    for (long long key = 0;key < KEYS;++key)
  ...
  for (long long lpos = 0;lpos < primes_num;++lpos) {
    ...
    printf(" 2addskip");
    for (long long t = 0;t < TIMINGS;++t)
      printf(" %lld",bench_isogeny[2][lpos][t]);
    printf("\n");
  }
